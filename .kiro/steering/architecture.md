...
- `app`:`docker`/`nginx`/`mysql` 能力**用例入口** `dispatch(body)` 已建立——web 能力 handler 一律经 `app::<cap>::dispatch`(web→app→infra),不再直接调 `infra::<cap>::web_dispatch`(架构测试已在 `web` 禁 `web_dispatch` token 锁死)。**nginx 全部只读 op + 两个可清晰拆分的写 op 已上移** app 层:只读 `get_settings`/`info`/`list_sites`/`list_named_certs`/`list_access`/`list_containers`/`list_dirs`/`list_ops`/`op_log`;写 `set_tuning`/`set_default_site`(纯校验/合并下沉 `domain::nginx::merge_http_tuning`/`build_default_site` + 单测,副作用 `infra::nginx::apply_*`,编排 `app::nginx`,`ERR_CODE:` 走 §6 过渡通道,行为零变化)。
...
- 能力**内部**竖切剩余写 op(nginx 的 `add_site`/`update_site`/`remove_site`/`create_cert`/`renew_cert`/`delete_cert`/`save_access`/`delete_access`/`setup`,以及 docker/mysql 全部 op):这些写 op 的校验与 infra 状态**深度交织**——例如 `site_from_req` 不纯(读取 access 列表 `load_access`、`valid_local_root` 触碰文件系统、生成随机 id),无法像 `set_tuning` 那样干净地把纯规则抽进 domain。**属高改动面 + 直接生成线上 nginx 配置、本地无法运行期验证**,须在真机逐 op 验证(站点新增/编辑/删除、证书签发、访问列表),不要盲改。建议先验证已上移的只读 + `set_tuning`/`set_default_site` 无回归,再逐个推进。
