...
- `app`:`docker`/`nginx`/`mysql` 能力**用例入口** `dispatch(body)` 已建立——web 能力 handler 一律经 `app::<cap>::dispatch`(web→app→infra),不再直接调 `infra::<cap>::web_dispatch`(架构测试已在 `web` 禁 `web_dispatch` token 锁死)。**nginx 已全量 op 路由进 `app::nginx`**(`infra::nginx::web_dispatch` 已删除):只读 op 在 app 直接投影;`set_tuning`/`set_default_site` 纯校验/合并下沉 `domain::nginx`(+单测);其余写 op(`add_site`/`update_site`/`remove_site`/`create_cert`/`renew_cert`/`delete_cert`/`save_access`/`delete_access`/`setup`/`reload`/`dismiss_op`)由 app 解析 `Req` 后调 `infra::nginx::op_*` 适配器执行,行为零变化。**docker/mysql** 经 `app::<cap>::dispatch` 路由,其 op 无可干净抽出的纯领域规则(校验与 bollard/容器 exec 状态交织),**权威 per-op match 保留在 infra dispatcher**(在 app 重写其大 match 会有漏路由/改行为风险),故 app 委托 `infra::<cap>::run_op` 执行。能力分层迁移到此完成。**docker/mysql 已升级**:`app::<cap>` 解析请求 DTO + 拥有 op 路由(registry op 在 app 处理),把 DB/容器 op 委托给 `infra::<cap>::run_op`(权威 per-op match + managed-service 守卫保留在 infra,避免在 app 重抄大 match 漏路由);`web_dispatch` 已删除。mysql 解析 `contracts::mysql::Req`;docker 因 `Req` 字段多+内嵌子结构,暂解析 `infra::docker::Req`(app→infra 既有模式,避免逐字搬运 45 字段的转写风险)。
...
- nginx 写 op 的**纯校验下沉**:`add_site`/`update_site` 等的字段校验目前仍在 `infra::nginx`(`site_from_req` 不纯——读 access 存储 + 触碰文件系统 + 生成 id),已通过 `app::nginx` 解析 `Req` 后委托 infra 适配器执行(行为零变化);后续可在真机验证下,把其中**纯字段校验**逐步抽到 `domain::nginx`(如已做的 `merge_http_tuning`/`build_default_site`),infra 状态检查保留。属可选精炼,非阻塞。
