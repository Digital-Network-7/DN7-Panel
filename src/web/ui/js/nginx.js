...
function renderNginx(v) {
  ...
  op('nginx', { op: 'info' }).then((info) => {
    ...
    v.innerHTML = `<div class="row" style="margin-bottom:14px"><span class="chip on">${tr('ng.running')}</span><span class="sp" style="flex:1"></span><button class="btn sm" id="ngAdd">${tr('ng.add_site')}</button><button class="btn sec sm" id="ngCert">${tr('ng.ssl_cert')}</button><button class="btn sec sm" id="ngReload">${tr('ng.reload')}</button><button class="btn sec sm" id="ngRef">${tr('ng.refresh')}</button></div><div id="ngSites">${loading()}</div>`;
    $('ngRef').onclick = () => renderNginx(v);
    $('ngReload').onclick = () => op('nginx', { op: 'reload' }).then(() => toast(tr('ng.reloaded'), 'ok')).catch((e) => toast(e.message, 'err'));
    ...
    op('nginx', { op: 'list_sites' }).then((d) => {
      ...
      sites.forEach((s) => {
        ...
        h += `<tr><td><b>${esc(s.server_name)}</b></td><td class="mut">${esc(kindLabel(s.kind))}</td><td class="mono" style="font-size:12px">${target}</td><td>${s.ssl ? `<span class="chip on">${tr('ng.yes')}</span>` : `<span class="chip">${tr('ng.no')}</span>`}</td><td class="act"><button class="btn sm danger" data-rm="${esc(s.id)}">${tr('ng.delete')}</button></td></tr>`;
      });
...
function ngAddSite(reload) {
  modal(tr('ng.add_site_title'), `
    ...
    <div class="ftab-pane" data-p="ssl">
      ...
      <div class="hidden" id="nsCertWrap" style="margin-top:12px"><label class="lbl">${tr('ng.cert_mode')}</label><select id="nsCert" class="field"><option value="named">${tr('ng.cm_named')}</option><option value="le">${tr('ng.cm_le')}</option><option value="self">${tr('ng.cm_self')}</option><option value="manual">${tr('ng.cm_manual')}</option></select></div>
      ...
      <div class="full hidden" id="nsManual" style="margin-top:10px"><label class="lbl">${tr('ng.cert_pem')}</label><textarea id="nsCertPem" class="field" rows="3"></textarea><label class="lbl" style="margin-top:8px">${tr('ng.key_pem')}</label><textarea id="nsKeyPem" class="field" rows="3"></textarea></div>
    </div>
    <div class="row" style="justify-content:flex-end;margin-top:16px"><button class="btn" id="nsGo">${tr('ng.create')}</button></div>
    ...
    const kindFields = () => {
      ...
      if (k === 'proxy_host') {
        ...
        $('nsKindFields').innerHTML = `
          ...
          <div class="dropz" id="nsDrop"><b>${tr('ng.drop_a')}</b>${tr('ng.drop_b')}<br/><span style="font-size:11.5px">${tr('ng.drop_sub')}</span></div>
    ...
    $('nsGo').onclick = async () => {
      ...
      else { body.root = $('nsRoot').value.trim(); if (!body.root) return toast(tr('ng.need_static_dir'), 'err'); if (!staticUpload.mode) return toast(tr('ng.need_upload'), 'err'); }
      if (body.ssl) {
      ...
      try {
        if (k === 'static') { $('nsJob').innerHTML = `<div class="mut">${tr('ng.uploading')}</div>`; await uploadStatic(body.root, staticUpload); }
      } catch (e) { toast(tr('ng.upload_failed') + '：' + e.message, 'err'); $('nsJob').innerHTML = ''; $('nsGo').disabled = false; return; }
      op('nginx', body).then((r) => {
        if (r.op_id) renderJob($('nsJob'), 'nginx', r.op_id, '', { onDone: () => { toast(tr('ng.site_created'), 'ok'); close(); reload(); }, onError: () => { $('nsGo').disabled = false; } });
        else { toast(tr('ng.site_created'), 'ok'); close(); reload(); }
...
function ngCerts() {
  modal(tr('ng.cert_mgr'), `
    <div class="seg" id="ngCertTabs" style="margin-bottom:14px">
      <button class="on" data-t="standalone">${tr('ng.cert_lib')}</button>
      <button data-t="sites">${tr('ng.site_certs')}</button>
    </div>
    ...
    const loadStandalone = () => op('nginx', { op: 'list_named_certs' }).then((d) => {
      ...
      document.querySelectorAll('#ngCertBody [data-del]').forEach((b) => b.onclick = async () => { if (await confirmDanger(tr('ng.confirm_del_cert', { name: b.dataset.del }))) op('nginx', { op: 'delete_cert', cert_name: b.dataset.del }).then(() => { toast(tr('common.deleted'), 'ok'); loadStandalone(); }).catch((e) => toast(e.message, 'err')); });
    }).catch((e) => { $('ngCertBody').innerHTML = `<p class="err">${esc(e.message)}</p>`; });
    const loadSites = () => op('nginx', { op: 'list_certs' }).then((d) => {
      const certs = d.certs || [];
      if (!certs.length) { $('ngCertBody').innerHTML = `<div class="empty">${tr('ng.no_sites_certs')}</div>`; return; }
      let h = `<table class="optable"><tr><th>${tr('ng.col_domain')}</th><th>${tr('ng.col_mode')}</th><th>${tr('ng.col_expire')}</th><th>${tr('ng.col_status')}</th><th class="act">${tr('ng.col_actions')}</th></tr>`;
      certs.forEach((c) => {
        const modeLabel = c.cert_mode === 'named' ? tr('ng.cm_named_short') : ({ le: tr('ng.mode_le'), self: tr('ng.mode_self'), manual: tr('ng.mode_manual') }[c.cert_mode] || (c.ssl ? c.cert_mode : '—'));
        const status = !c.ssl ? `<span class="chip">${tr('ng.not_enabled')}</span>` : (c.has_cert ? `<span class="chip on">${tr('ng.issued')}</span>` : `<span class="chip warn">${tr('ng.missing')}</span>`);
        h += `<tr><td><b>${esc(c.server_name)}</b></td><td class="mut">${esc(modeLabel)}</td><td class="mono" style="font-size:12px">${esc(c.not_after || '-')}</td><td>${status}</td><td class="act"><button class="btn sm" data-id="${esc(c.id)}" data-name="${esc(c.server_name)}">${tr('ng.config_cert')}</button></td></tr>`;
      });
...
function ngCreateCert(reload) {
  modal(tr('ng.create_cert'), `
    <div class="formgrid">
      ...
      <div class="full"><label class="lbl">${tr('ng.cert_mode')}</label><select id="ccMode" class="field"><option value="self">${tr('ng.cm_self')}</option><option value="le">${tr('ng.cm_le')}</option><option value="manual">${tr('ng.cm_manual')}</option></select></div>
...
function ngSetCert(siteId, name, reload) {
  modal(tr('ng.config_cert_title') + name, `
    <label class="lbl">${tr('ng.cert_mode')}</label>
    <select id="scMode" class="field" style="margin-bottom:14px">
      <option value="named">${tr('ng.cm_named')}</option>
      <option value="le">${tr('ng.cm_le')}</option>
      <option value="self">${tr('ng.cm_self')}</option>
      <option value="manual">${tr('ng.cm_manual')}</option>
    </select>
    <div class="hidden" id="scNamedWrap"><label class="lbl">${tr('ng.select_cert')}</label><select id="scNamed" class="field" style="margin-bottom:14px"></select></div>
    <div class="hidden" id="scManual"><label class="lbl">${tr('ng.cert_pem')}</label><textarea id="scCert" class="field" rows="4"></textarea><label class="lbl" style="margin-top:8px">${tr('ng.key_pem')}</label><textarea id="scKey" class="field" rows="4"></textarea></div>
    <p class="mut" style="font-size:12px;margin-top:6px" id="scHint"></p>
    <div class="row" style="justify-content:flex-end;margin-top:14px"><button class="btn" id="scGo">${tr('ng.apply')}</button></div>
    ...
    const sync = () => {
      ...
      $('scHint').textContent = m === 'le' ? tr('ng.hint_le') : m === 'self' ? tr('ng.hint_self') : m === 'named' ? tr('ng.hint_named') : tr('ng.hint_manual');
    };
    ...
    $('scGo').onclick = () => {
      ...
      else if (mode === 'named') { body.cert_name = $('scNamed').value; if (!body.cert_name) return toast(tr('ng.need_named_lib'), 'err'); }
      $('scGo').disabled = true; $('scJob').classList.remove('hidden'); $('scJob').innerHTML = `<div class="mut">${tr('ng.submitting')}</div>`;
      op('nginx', body).then((r) => {
        if (r.op_id) renderJob($('scJob'), 'nginx', r.op_id, '', { onDone: () => { toast(tr('ng.cert_configured'), 'ok'); close(); reload(); }, onError: () => { $('scGo').disabled = false; } });
        else { toast(tr('ng.cert_configured'), 'ok'); close(); reload(); }
