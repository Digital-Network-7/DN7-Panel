//! MySQL instance install (detached): pull image, create volume + container,
//! wait-ready, persist manifest. Also the shared install/create specs + helpers.
use super::*;
...
pub(crate) async fn run_install_detached(op_id: &str, spec: InstallSpec) -> Result<()> {
    ...
    let root_enc = crate::infra::support::crypto::encrypt(&password)
