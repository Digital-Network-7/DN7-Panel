...
//! issuance). A long-running op is created, streams progress `lines`, and is
//! polled by the web console via `list_ops` / `op_log` until done/error.
//!
//! Split out of the parent `nginx` module to keep concerns separate; the
//! progress-line/percent semantics match the docker + mysql registries.
...
pub(super) fn op_running(op_id: &str) -> bool {
    ops()
        .lock()
        .ok()
        .and_then(|m| m.get(op_id).map(|o| o.status == "running"))
        .unwrap_or(false)
}
